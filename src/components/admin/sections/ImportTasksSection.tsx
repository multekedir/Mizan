...
export function ImportTasksSection() {
  const importJson = useTaskStore((s) => s.importFromJson);
  const importCsv = useTaskStore((s) => s.importFromCsv);
  const clearToday = useTaskStore((s) => s.clearToday);
  ...
  return (
    <div>
      <p className="mb-3 text-xs leading-relaxed text-mizan-text/50">
        <strong>JSON:</strong> {'[{"title":"…","assignee":"Aisha","recurring":false}]'}
        <br />
        <strong>CSV:</strong> title, assignee, recurring
      </p>
      <textarea
        value={text}
        onChange={(e) => setText(e.target.value)}
        className="kiosk-allow-select min-h-[160px] w-full rounded-2xl border border-mizan-surfaceSoft bg-mizan-bg p-4 font-mono text-xs outline-none focus:ring-2 focus:ring-mizan-success"
