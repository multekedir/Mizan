...
from kb import kb
from models import ChatRequest, ChatResponse, KBDocument, SuggestedTask, GoalSuggestion
...
def _parse(
    raw: str,
    members: list[str],
) -> tuple[str, list[SuggestedTask], list[str], "GoalSuggestion | None"]:
    text = raw.strip()
    if text.startswith("```"):
    ...
    return message, tasks, updates, goal
...
async def _prepare_chat(
    ...
    is_no_task = (
        not should_generate_tasks(req.message)
        or (is_birthday_without_date(req.message) and not _birthday_date_in_notes(notes))
    )
    cfg = settings.ollama
    if is_no_task:
    ...
    use_json = settings.assistant.response_format == "json" and not is_no_task
    return _ChatPrep(is_no_task, model, member_list, messages, use_json)
...
async def chat(
    ...
    payload: dict = {
        ...
        "stream": False,
        "options": {"temperature": 0.2, "num_predict": 300 if prep.is_no_task else 2000},
    }
    if prep.use_json:
        payload["format"] = "json"
    ...
    if prep.use_json:
        message, tasks, memory_updates, goal = _parse(raw, prep.member_list)
        tasks = _postprocess_tasks(
            tasks, req.message,
            existing=[t.title for t in req.live_context.current_tasks],
            history_titles=req.live_context.completed_history,
            members=prep.member_list,
            force_today=_is_today_context(req.message, history),
        )
    else:
        message, tasks, memory_updates, goal = raw.strip(), [], [], None

    mode = "light_day" if is_light_day_request(req.message) else ("tasks" if tasks else "chat")
    return ChatResponse(
        message=message, tasks=tasks, mode=mode, memory_updates=memory_updates, goal=goal,
    )
...
async def chat_stream(
    ...
    if prep.use_json:
        message, tasks, memory_updates, goal = _parse(accumulated, prep.member_list)
        tasks = _postprocess_tasks(
            tasks, req.message,
            existing=[t.title for t in req.live_context.current_tasks],
            history_titles=req.live_context.completed_history,
        ...
        message, tasks, memory_updates, goal = accumulated.strip(), [], [], None
